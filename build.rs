...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (idf_target, linker) = match env::var("TARGET")?.as_ref() {
        "xtensa-esp32-none-elf" => {
            ...
            (
                "esp32".to_string(),
                env::var("RUSTC_LINKER").unwrap_or("xtensa-esp32-elf-ld".to_string()),
            )
        }
        target => {
    ...
    let component_includes =
        globwalk::GlobWalkerBuilder::from_patterns(&idf_path, &["components/**/include"])
            .build()?
            .filter_map(Result::ok)
            .map(|d| d.into_path());

    let component_additional_includes =
        globwalk::GlobWalkerBuilder::from_patterns(&idf_path, &["components/*/component.mk"])
            .build()?
            .filter_map(Result::ok)
            .flat_map(|makefile| {
                ...
                let mut contents = read_to_string(&path)
                    .expect("failed reading component.mk")
                    .replace("$(info ", "$(warn ");
    ...
    let mut includes = component_includes
        .chain(component_additional_includes)
